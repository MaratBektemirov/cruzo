...
function matchOp(input: string, i: number): string {
  for (const op of ops) if (input.startsWith(op, i)) return op;
  return null;
}
  ...
  while (i < input.length) {
    ...
    if (c === "'" || c === '"') {
      ...
      while (i < input.length) {
        ...
        if (ch === "\\") {
          if (i >= input.length) {
            throw new Error(`Unterminated escape sequence in string at ${start}`);
          }
          const nxt = input[i++];
          s += nxt === "n" ? "\n" : nxt === "t" ? "\t" : nxt;
          continue;
    ...
    if (isDigit(c) || (c === "." && isDigit(input[i + 1] || ""))) {
      let s = "";
      while (i < input.length && (isDigit(input[i]) || input[i] === ".")) s += input[i++];
...
export class Bytecode {
  ...
  fmtArg(op: OP, a: number, wide?: number) {
    switch (op) {
      ...
      case OP.LOAD_ID:
        return [this.idNames[a] ?? `<bad id[${a}]>`, ""];

      case OP.GET_PROP:
      case OP.GET_PROP_KEEP:
        return [show(this.consts[a]), ""];
  ...
  getSafeErrorMsg(obj: any, prop: string) {
    return `TemplateTypeError: Cannot read properties of ${obj === null ? "null" : "undefined"} (reading '${prop}')`
  }
  ...
  run(
    ...
    const callN = (fnIndex: number, thisArg: any, argc: number) => {
      const fn = stack[fnIndex];

      if (typeof fn !== "function") {
        throw new Error(`Cannot call undefined or non-function value: ${fn}`);
      }
      if (!argc) return fn.call(thisArg);
      const args = stack.slice(sp - argc, sp);
    ...
    while (ip < code.length) {
      ...
      switch (op) {
        ...
        case OP.LOAD_THIS:
          thisArg ??= template.getThisArg(allowRxLink);
          stack[sp++] = thisArg;
          break;
        ...
        case OP.GET_PROP: {
          ...
          const obj = stack[--sp];
          if (obj == null) {
            template.handleSafeError(this.getSafeErrorMsg(obj, prop), this.expr);
            stack[sp++] = undefined;
          } else {
            stack[sp++] = obj[prop];
          }
        ...
        case OP.GET_INDEX: {
          ...
          const obj = stack[--sp];
          if (obj == null) {
            template.handleSafeError(this.getSafeErrorMsg(obj, idx+''), this.expr);
            stack[sp++] = undefined;
          } else {
            stack[sp++] = obj[idx];
          }
        ...
        case OP.CALL_FN: {
          ...
          thisArg ??= template.getThisArg(allowRxLink);
          const res = callN(fnIndex, thisArg, argc);
          sp = fnIndex;
          stack[sp++] = res;
          break;
        }
        ...
        case OP.CALL_METHOD: {
          ...
          const res = callN(fnIndex, obj, argc);
          sp = objIndex;
          stack[sp++] = res;
          break;
        }
...
export class VMProgramCompiler {
  ...
  private parseBinaryCounted(
    next: () => void,
    ops: readonly string[],
    emitFor: (op: string) => void
  ) {
    next();
    let t = this.peek();

    if (this.canCount() && t.t === "op" && ops.includes(t.v)) {
      let terms = 1;
      while (t.t === "op" && ops.includes(t.v)) {
  ...
  private parseShortCircuit(next: () => void, token: "||" | "&&", jumpOp: OP) {
    next();
    let t = this.peek();

    const count = this.canCount() && t.t === "op" && t.v === token;
    let terms = 1;

    while (t.t === "op" && t.v === token) {
      this.next();
      const jKeep = this.emitJump(jumpOp);
      next();
    ...
    if (count) this.exprCount = terms;
  }

  private parseNullishChain(next: () => void) {
    next();
    let t = this.peek();

    const count = this.canCount() && t.t === "op" && t.v === "??";
    let terms = 1;
  ...
  private parseTernary() {
    ...
    const t = this.peek();
    if (t.t === "op" && t.v === "?") {
      if (this.canCount()) this.exprCount = 3;
  ...
  private parseOr() {
    this.parseShortCircuit(() => this.parseAnd(), "||", OP.JMP_IF_TRUE_KEEP);
  }

  private parseAnd() {
    this.parseShortCircuit(() => this.parseEq(), "&&", OP.JMP_IF_FALSE_KEEP);
  }
  ...
  private parseAdd() {
    this.parseBinaryCounted(
      () => this.parseMul(),
      ["+", "-"],
      (op) => this.emit(op === "+" ? OP.BIN_ADD : OP.BIN_SUB)
    );
  }
  ...
  private parseMul() {
    this.parseBinaryCounted(
      () => this.parseUnary(),
      ["*", "/", "%"],
      (op) => this.emit(op === "*" ? OP.BIN_MUL : op === "/" ? OP.BIN_DIV : OP.BIN_MOD)
    );
  }
  ...
  private parsePostfix() {
    ...
    const flushObjFnPair = () => {
      if (!hasObjFnPair) return;
    ...
    const emitProp = (prop: string) => {
      const propIdx = this.c(prop);
      const la = this.peek();
      const isCall = la.t === "punc" && la.v === "(";
      this.emit(isCall ? OP.GET_PROP_KEEP : OP.GET_PROP, propIdx);
      hasObjFnPair = isCall;
    };
    ...
    const emitIndex = () => {
      const la = this.peek();
      const isCall = la.t === "punc" && la.v === "(";
      this.emit(isCall ? OP.GET_INDEX_KEEP : OP.GET_INDEX);
      hasObjFnPair = isCall;
    };
