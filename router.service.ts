...
class RouterService extends AbstractService {
  ...
  public update(ignoreRedirectRules = false) {
    ...
    if (this.pathname$.value !== url.pathname) {
    ...
    if (this.hash$.value !== url.hash) {
      this.hash$.update(url.hash);
      this.scrollToHashElement();
    }

    if (this.search$.value !== url.search) {
  ...
  public scrollToHashElement() {
    ...
    if (this.hash$.value) {
      document.querySelector(this.hash$.value)?.scrollIntoView({ behavior: "instant", block: "start" });
    }
  }
  ...
  public pushHistory(href: string) {
    ...
    if (this.pathname$.value === url.pathname && this.hash$.value === url.hash && this.search$.value === url.search) return;
  ...
  private applyComponentRulesForUrl(componentRules: Rule[]) {
    ...
    for (const rule of prev) {
      ...
      componentsRegistryService.removeComponents(rule.components);
    }
